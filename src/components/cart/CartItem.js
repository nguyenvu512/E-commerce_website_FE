...
const CartItem = ({
  ...
  const updateCartItemQuantity = (newQuantity) => {
    ...
    axios
      .put(`http://localhost:8082/cart-service/cart/${accountID}`, payload, {
        headers: {
  ...
  const deleteCartItem = () => {
    axios
      .delete(
        `http://localhost:8082/cart-service/cart/delete-item/${accountID}?productID=${productID}&color=${color}&size=${size}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      <div className="cart-item-quantity-totalPrice">
        ...
        <div className="cart-item-totalPrice">
          <span style={{ fontWeight: "bold" }}>
            {MoneyFormat(productPrice * quantityProduct)}
          </span>
        </div>
      </div>
