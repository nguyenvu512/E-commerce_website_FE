...
const Cart = () => {
  ...
  // token test
  const token =
    "eyJhbGciOiJIUzI1NiJ9..gfpdWHR_oKvpxWk906ujZFwTTIHlipADiG2A6P5fbME";
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8082/cart-service/cart/get/${accountID}`, {
        headers: {
  ...
  const handleChangeQuantity = (productID, newQuantity) => {
    ...
    const newTotalPrice = cartItemsSelected.reduce(
      (acc, item) =>
        item.productID === productID
          ? acc + item.productPrice * newQuantity
          : acc + item.productPrice * item.quantity,
      0
    );
    setTotalPrice(newTotalPrice);
